# chess.py - Python port (compact board style)
# Saved path: /mnt/data/chess.py
...
# counts of pieces (used for 50-move rule detection)
piece_count = 0
piece_count_previous = 32
...
def print50():
    print("\033[36m50-MOVE RULE DRAW\033[0m")
...
def printBoard():
    ...
    for row in range(8):
        rank = 8 - row
        print(f"\x1b[30m\x1b[46m {rank} \x1b[0m\x1b[30m", end="")

        for col in range(8):
            if row % 2 == 0:
                bg = "\x1b[47m" if col % 2 == 0 else "\x1b[100m"
            else:
                bg = "\x1b[100m" if col % 2 == 0 else "\x1b[47m"

            print(bg + PIECES[board[row][col]], end="")

        print(f"\x1b[30m\x1b[46m {rank} \x1b[0m")
...
def isCastle(Board, r1, c1, r2, c2):
    ...
    # Black side castling (top of board in C code)
    if r1 == 0 and c1 == 4:
        if black_king_moved:
            return False
...
def playerHasValidMove():
    ...
    Checks:
      ...
      • Updates tempboard and piece_count.
      • Sets checkmate/stalemate flags.
    """
    global tempboard, board
    global piece_count, piece_count_previous
    global checkmate, stalemate
    global player

    piece_count = 0

    # sync tempboard and count pieces
    for i in range(8):
        for j in range(8):
...
def main():
    global player, inactive_moves, checkmate, stalemate, resigned
    global piece_count, piece_count_previous
    ...
    while playerHasValidMove():
        # inactive move tracking (50-move rule)
        if piece_count_previous == piece_count:
            inactive_moves += 1
        else:
            inactive_moves = 1
        piece_count_previous = piece_count
        ...
        # normalize and parse moves like "e2e4"
        if len(move) == 4 and move[0].isalpha() and move[2].isalpha() and move[1].isdigit() and move[3].isdigit():
            ...
            if isValidMove(board, start_row, start_col, end_row, end_col):
                ...
                if makeMove(board, start_row, start_col, end_row, end_col):
                    ...
                    # update moved-piece flags (mirror C logic)
                    moved_piece = board[end_row][end_col]
                    if moved_piece == 5 or moved_piece == -5:
                        # note: player was toggled in makeMove; follow C logic
                        if not player:
                            white_king_moved = 1
                        else:
                            black_king_moved = 1
